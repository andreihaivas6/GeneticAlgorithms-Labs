...
#define N 30
#define prec 4
#define PC 0.1       // probabilitate cross_over // 0.1 0.2 
#define PM 0.001  // probabilitate mutatie    // 0.001 0.01/0.005
...
float michalewics(float* res)
{
    float result = 0;
    for (int i = 0; i < N; i++)
        result += sin(res[i]) * pow(sin(((double)i * res[i] * res[i]) / PI), 10); // 10 20
    return -result;
}
...
void evaluationFitness(int func)
{
    ...
    if (func == 1)
        ...
        {
            f(pop[i], resFit);
            auto aux = 1 / (michalewics(resFit) + N );
            popFit[i] = pow(aux, 2);// 
        }
}
